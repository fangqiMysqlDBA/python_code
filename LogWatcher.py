...
class LogWatcher(object):
    ...
    Example:
    >>> def callback(filename, lines, other):
    ...     print filename, lines
    ...
    >>> l = LogWatcher("/var/log/", callback)
    >>> l.loop()
    """
    def __init__(self, folder, callback, extensions=["log"], tail_lines=0,other={}):
        """Arguments:
        (str) @folder:
            the folder to watch
        (callable) @callback:
        ...
        for id, file in self.files_map.iteritems():
            ...
            if tail_lines:
                ...
                if lines:
                    self.callback(file.name, lines, self.other)
    def __del__(self):
        self.close()
    def log_end(self):
        try:
            filesize = os.path.getsize(self.other["slow_log"])
            if self.other['slow_size'] > filesize:
                self.other["exit"] = 1
                #print self.other["exit"]
            else:
                self.other['slow_size'] = filesize
        except Exception ,err:
            #print err
            pass
        try:
            filesize = os.path.getsize(self.other["mysql_log"])
            if self.other["mysql_size"] > filesize:
                self.other["exit"] = 1
                #print self.other["exit"]
            else:
                self.other['mysql_size'] = filesize
        except Exception ,err:
            #print err
            pass
